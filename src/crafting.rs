...
use crate::{
    cache::InMemoryCache,
    market::{get_cheapest_combination, minimum_price_per_unit, ItemListing},
};
...
pub(crate) struct ProfitCalcError {}

pub(crate) async fn calc_profit(
    recipe_id: usize,
    world: String,
    quantity: usize,
    item_data: &ItemData,
    cache: &InMemoryCache,
) -> Result<(f32, Vec<ItemListing>), ProfitCalcError> {
    let recipe = item_data.recipes.iter().find(|r| r.id == recipe_id);
    let mut cost: f32 = 0.;

    match recipe {
        Some(r) => {
            let mut req_items = Vec::new();
            for (item_id, ingredient_amount) in r.ingredients.clone() {
                let listings =
                    get_cheapest_combination(item_id, &world, &cache, ingredient_amount * quantity)
                        .await;
                req_items.append(&mut listings.clone());
                cost += listings.iter().map(|l| l.total_price).sum::<usize>() as f32;
            }
            let sale_price = minimum_price_per_unit(r.result_item_id, &world, cache).await;
            let profit = (sale_price * r.result_item_quantity as f32 * quantity as f32) - cost;
            debug!("profit: {profit} | cost: {cost} | quantity: {quantity} | sale price (ppu): {sale_price} | recipe: {r:?}");
            Ok((profit, req_items))
        }
        None => Err(ProfitCalcError {}),
    }
}
